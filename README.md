...
The `scripts/build.js` will scan these files an exclude the JSON-LD contexts and write it to `dist/schema.org` or `dist/schema.pending.b-ox.org`. This output is used on our sites. The build also writes the file `dist/index.json` that contains a list of all types and the number of objects / attribute and enumerations. You can start the script typing `npm run build` in your projects root directory.
